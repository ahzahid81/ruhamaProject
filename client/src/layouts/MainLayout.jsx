...
const navGroups = [
  ...
  },
  {
    ...
    items: [
      ...
      { name: "Student-wise Fees", path: "/fees/student-wise", icon: UserCheck, roles: ["admin"] },
    ],
  },
  {
    label: "Examination",
      ...
      {sidebarOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <div className="absolute left-0 top-0 h-full w-72 bg-white shadow-2xl flex flex-col">
            <button
              onClick={() => setSidebarOpen(false)}
              className="absolute top-4 right-3 z-10 p-2 rounded-xl text-gray-400 hover:bg-gray-100 transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
